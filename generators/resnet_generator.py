...
class resnet_generator(nn.Module):
    def __init__(self, d=128):
        ...
        self.last = nn.Conv2d(self.d, 3, kernel_size = (3, 3), padding = 1)
    
    def forward(self, z):
        x = self.lin(z).view(-1, self.d, 4, 4)
        x = self.postlin(x)
        x = torch.nn.functional.relu(self.resBlock1(x) + self.shortCut1(x))
        x = torch.nn.functional.relu(self.resBlock2(x) + self.shortCut2(x))
        x = torch.nn.functional.relu(self.resBlock3(x) + self.shortCut3(x))
        return self.last(x)
